...
rows, cols = df.shape
product = df.id.unique().shape[0]

print(f'Dataset Shape\n\nNumber of rows : {rows}\nNumber of columns: {cols}\nNumber of different types of products : {product}')
df.info()
...
convert_to_datetime(df, 'year')
convert_to_datetime(df, 'updated')
convert_to_string(df, 'name')
convert_to_string(df, 'category')
convert_to_string(df, 'gender')
convert_to_string(df, 'currentlyactiv')
print("type :")
df.info()
...
#load_data_to_postgres(result, 'postgresql://postgres:123456127.0.0.1:5432/ETL_product')
load_data_to_postgres(result, 'product', 'postgresql://postgres:123456@127.0.0.1:5432/ETL_product')