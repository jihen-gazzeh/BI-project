...
def missing_value_reporter(data):
    ...
    # Update the counts after dropping empty columns
...
input_file_name = "order.xlsx"

# Extract data
raw = extract_data(directory_path, input_file_name)
df = raw.copy()

# Checking clean shape and nº of customers
rows, cols = df.shape
customers = df.customerid.unique().shape[0]
print(f'Dataset Shape\n\nNumber of rows : {rows}\nNumber of columns: {cols}\nNumber of different customers : {customers}')
# Checking clean shape and nº of shipping address id
rows, cols = df.shape
shipping_address = df.shippingaddressid.unique().shape[0]
print(f'Dataset Shape\n\nNumber of rows : {rows}\nNumber of columns: {cols}\nNumber of different addresses : {shipping_address}')

#Convert to date
convert_to_datetime(df, 'created')
convert_to_datetime(df, 'ordertimestamp')

#Missing values
result, missing_report = missing_value_reporter(data=df)
print("\nMissing values report:")
print(missing_report)
print("Data after dropping rows if exists:")
print(result)
...
filtered_data = tmp
load_data_to_postgres(filtered_data, 'Order', 'postgresql://postgres:0000@127.0.0.1:5432/ETL')
